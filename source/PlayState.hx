...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (SONG == null)
			SONG = Song.loadFromJson('confrontance');
		...
		switch (SONG.song.toLowerCase())
		{
			case 'the potato warrior':
				dialogue = CoolUtil.coolTextFile(Paths.txt('the potato warrior/dialogue'));
		}

		switch (SONG.song.toLowerCase())
		{
			case 'agro nerd' | 'confrontance' | 'the potato warrior':
				{
					defaultCamZoom = 0.9;
					curStage = 'lmanburg';

					var bg:FlxSprite = new FlxSprite(-300, -225).loadGraphic(Paths.image('lmanburg/lmanburg', 'technoWeek'));
					bg.antialiasing = true;
					bg.scrollFactor.set(0.9, 0.9);
					bg.active = false;
					add(bg);

					if(SONG.song.toLowerCase() == 'the potato warrior')
					{
						...
						var fireFrames:FlxAtlasFrames = Paths.getSparrowAtlas('lmanburg/fire', 'technoWeek');


						for (i in 0...25)
						{
							var fire:FlxSprite = new FlxSprite(-25 + i * 400, 570);
							fire.frames = fireFrames;
							fire.animation.addByPrefix('fire', 'fire', 12);
							fire.animation.play('fire');
							fire.antialiasing = false;
							fire.scrollFactor.set(0.6, 0.6);
							fire.setGraphicSize(Std.int(fire.width * 25));
							add(fire);
						}

						for (i in 0...25)
						{
							var fire:FlxSprite = new FlxSprite(-15 + i * 400, 670);
							fire.frames = fireFrames;
							fire.animation.addByPrefix('fire', 'fire', 12);
							fire.animation.play('fire');
							fire.antialiasing = false;
							fire.scrollFactor.set(0.75, 0.75);
							fire.setGraphicSize(Std.int(fire.width * 25));
							add(fire);
						}
						for (i in 0...25)
							{
								var fire:FlxSprite = new FlxSprite(-15 + i * 400, 770);
								fire.frames = fireFrames;
								fire.animation.addByPrefix('fire', 'fire', 12);
								fire.animation.play('fire');
								fire.antialiasing = false;
					...
					var stageFront:FlxSprite = new FlxSprite(-150, 600).loadGraphic(Paths.image('lmanburg/platform', 'technoWeek'));
		...
		switch (SONG.player2)
		{
			...
			case 'technoblade' | 'technoblade-mad':
				camPos.x += 600;
				dad.y += 350;
		...
		healthBar.createFilledBar(0xFFFFBBCE, 0xFF31B0D1);
		// healthBar
		add(healthBar);

		var coolTextForSongStuffLol = new FlxText(4, healthBarBG.y
			+ 50, 0,
			SONG.song
			+ " "
		...
		coolTextForSongStuffLol.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		coolTextForSongStuffLol.scrollFactor.set();
		add(coolTextForSongStuffLol);

		if (FlxG.save.data.downscroll)
			coolTextForSongStuffLol.y = FlxG.height * 0.9 + 45;

		scoreTxt = new FlxText(FlxG.width / 2 - 235, healthBarBG.y + 50, 0, "ay caramba donde esta la biblioteca!", 20);
		...
		coolTextForSongStuffLol.cameras = [camHUD];
		if (loadRep)
	...
	function technoCutscene(?dialogueBox:DialogueBox):Void
	{
					...
					if (dialogueBox != null)
						{
							...
							if (SONG.song.toLowerCase() == 'the potato warrior')
							{
								...
								{
									FlxG.sound.play(Paths.sound('Technoblade_Never_Dies'), 1, false, null, true);
		
									new FlxTimer().start(0.3, function(swagTimer2:FlxTimer)
									{
										red.alpha -= 0.15;
				
										if (red.alpha > 0)
										{
											swagTimer2.reset(0.3);
										}
										else {
											remove(red);
											add(dialogueBox);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			// taken from a switch case we dont need because WE ONLY HAVE TECHNO WEEK AND NO WEEK 6 LOLLLLLLLLLLLLLLL
			...
			if (b)
			{
				switch (i)
				{
					...
					case 15:
						ranking += " D";
					case 16:
						ranking == " Technoblade Would Not Be Proud";
				}
				break;
			}
		}
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					FlxG.save.data.weekUnlocked = [true];
					FlxG.save.flush();
				}
				else
				{
